...
def init_db():
    ...
    conn.executescript("""
        CREATE TABLE IF NOT EXISTS payments (
            ...
            name TEXT, svc TEXT, src TEXT,
            grn REAL DEFAULT 0, usd REAL DEFAULT 0,
            rate REAL DEFAULT 0, comm REAL DEFAULT 0,
            status TEXT DEFAULT 'paid', comment TEXT DEFAULT ''
        );
        CREATE TABLE IF NOT EXISTS months (
            key TEXT PRIMARY KEY, year TEXT, month TEXT,
            label TEXT, plan REAL DEFAULT 150000, partial INTEGER DEFAULT 0
        );
...
def seed(conn):
    conn.executemany("INSERT OR IGNORE INTO months VALUES(?,?,?,?,?,?)", [
    ...
    conn.executemany("INSERT OR IGNORE INTO payments VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)", [
        ...
        ("h05","2025-11-18","2025","11","58mile.com.ua","seo","baza",29554,700,0.10,2955,"paid","Без бюджету ($250)"),
        ...
        ("h09","2025-12-22","2025","12","sanmarine.ua","smm","networking",25470,600,0.10,2547,"paid",""),
        ("h10","2026-01-06","2026","01","vista-health.com.ua","smm","baza",46805,1100,0.50,23403,"paid","SMM+Meta"),
        ("h11","2026-01-20","2026","01","kodvody.com.ua","google","fb_viktor",45465,1050,0.50,22733,"paid",""),
        ("h12","2026-01-20","2026","01","keepcul.com","meta","ilia",17280,400,0.50,8640,"paid",""),
        ("h13","2026-02-26","2026","02","avtopan.ua","complex","baza",29968,700,0.20,5994,"part","50%"),
        ("h14","2026-02-13","2026","02","clipnjoy.ca","meta","fb_viktor",13766,315,0.50,6883,"paid",""),
        ("h15","2026-02-20","2026","02","matras.kiev.ua","web","baza",1000,0,0.10,100,"paid",""),
        ...
        ("h19","2026-03-28","2026","03","autoon.kiev.ua","complex","baza",30914,700,0.20,7176,"paid",""),
        ("h20","2026-04-13","2026","04","Bedoin","meta","networking",21950,500,0.50,10975,"paid",""),
        ("h21","2026-04-20","2026","04","Malva","meta_google","baza",54293,1080,0.50,27147,"paid",""),
        ("h22","2026-04-27","2026","04","Vegera (1-а оплата)","meta","baza",8700,200,0.50,4350,"part",""),
        ("h23","2026-04-16","2026","04","repobona.com.ua","web","baza",13166,300,0.15,2015,"paid",""),
        ("h24","2026-04-21","2026","04","Tep.ua","meta","baza",19774,450,0.50,9887,"paid",""),
        ...
        ("h27","2026-05-08","2026","05","Maxcryptobybit","tg","ilia",31376,600,0.45,14119,"paid","600 EUR"),
        ("h28","2026-05-17","2026","05","blue chip","google","biz_club",20532,400,0.50,10266,"paid",""),
        ("h29","2026-05-21","2026","05","ektabud.pro","web","baza",35864,810,0.10,3586,"paid",""),
...
@auth
def get_pays():
    return jsonify(rs(get_db().execute("SELECT * FROM payments ORDER BY date DESC").fetchall()))
...
def create_month():
    ...
    db.commit()
    res = dict(db.execute("SELECT * FROM months WHERE key=?",(key,)).fetchone())
    res["partial"] = bool(res["partial"])
...
@auth
def get_expenses():
    return jsonify(rs(get_db().execute("SELECT * FROM expenses ORDER BY date DESC").fetchall()))
...
def create_expense():
    ...
    p = d["date"].split("-"); db = get_db()
    db.execute("INSERT INTO expenses VALUES(?,?,?,?,?,?,?,?,?)",
        (eid,d["date"],p[0],p[1],d["name"],d.get("type","lead_fb"),
         d.get("who","me"),float(d.get("amt",0)),d.get("comment","")))
    db.commit()
...
def update_expense(eid):
    ...
    db.execute("UPDATE expenses SET date=?,year=?,month=?,name=?,type=?,who=?,amt=?,comment=? WHERE id=?",
        (d["date"],p[0],p[1],d["name"],d.get("type","lead_fb"),
         d.get("who","me"),float(d.get("amt",0)),d.get("comment",""),eid))
    db.commit()
...
@auth
def get_pipeline():
    return jsonify(rs(get_db().execute("SELECT * FROM pipeline ORDER BY updated_at DESC").fetchall()))
...
def reset_data():
    conn = sqlite3.connect(DB_PATH)
    conn.executescript("DELETE FROM payments; DELETE FROM months; DELETE FROM expenses; DELETE FROM pipeline;")
    conn.commit()
    seed(conn)
    conn.close()
    return jsonify({"ok": True})
